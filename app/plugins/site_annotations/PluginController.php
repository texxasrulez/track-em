...
require_once __DIR__ . '/SiteAnnotationsService.php';

final class PluginController
{
    private SiteAnnotationsService $service;
    private string $pluginDir;

    public function __construct(string $pluginId, string $pluginDir)
    {
        $this->pluginDir = rtrim($pluginDir, '/\\');
        $this->service = new SiteAnnotationsService($pluginId, $pluginDir);
    }
    ...
    private function admin(): bool
    {
        ...
        $report = $this->service->report($config);
        $csrf = $this->service->csrfToken();
        $annotationTypes = $this->service->annotationTypes();
    ...
    private function requireAdminPost(): bool
    {
        ...
        if (
            !Security::rateLimit(
                'site_annotations_admin:' . Security::clientIpMasked(),
                60,
                30,
            )
        ) {
