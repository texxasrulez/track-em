...
require_once __DIR__ . '/GeoIntelService.php';

final class PluginController
{
    private GeoIntelService $service;
    private string $pluginDir;

    public function __construct(string $pluginId, string $pluginDir)
    {
        $this->pluginDir = rtrim($pluginDir, '/\\');
        $this->service = new GeoIntelService($pluginId, $pluginDir);
    }
    ...
    private function requireAdminPost(): bool
    {
        ...
        if (
            !Security::rateLimit(
                'geo_intel_admin:' . Security::clientIpMasked(),
                60,
                30,
            )
        ) {
