...
require_once __DIR__ . '/ContentGroupsService.php';

final class PluginController
{
    private ContentGroupsService $service;
    private string $pluginDir;

    public function __construct(string $pluginId, string $pluginDir)
    {
        $this->pluginDir = rtrim($pluginDir, '/\\');
        $this->service = new ContentGroupsService($pluginId, $pluginDir);
    }
    ...
    private function admin(): bool
    {
        ...
        $matchTypes = $this->service->matchTypes();
    ...
    private function requireAdminPost(): bool
    {
        ...
        if (
            !Security::rateLimit(
                'content_groups_admin:' . Security::clientIpMasked(),
                60,
                30,
            )
        ) {
