...
require_once __DIR__ . '/AnomalyDigestService.php';

final class PluginController
{
    private AnomalyDigestService $service;
    private string $pluginDir;

    public function __construct(string $pluginId, string $pluginDir)
    {
        $this->pluginDir = rtrim($pluginDir, '/\\');
        $this->service = new AnomalyDigestService($pluginId, $pluginDir);
    }
    ...
    private function requireAdminPost(): bool
    {
        ...
        if (
            !Security::rateLimit(
                'anomaly_digest_admin:' . Security::clientIpMasked(),
                60,
                30,
            )
        ) {
