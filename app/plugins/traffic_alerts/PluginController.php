...
require_once __DIR__ . '/TrafficAlertsService.php';

final class PluginController
{
    private string $pluginDir;
    private TrafficAlertsService $service;

    public function __construct(string $pluginId, string $pluginDir)
    {
        $this->pluginDir = rtrim($pluginDir, '/\\');
        $this->service = new TrafficAlertsService($pluginId, $pluginDir);
    }
    ...
    private function admin(): bool
    {
        ...
        $result = $this->service->runChecks($config, false);
        $state = $this->service->loadState();
        $alerts = $this->service->recentAlerts(20);
        $channels = $this->service->channelAvailability($config);
    ...
    private function rebuild(): bool
    {
        ...
        $config = $this->service->loadConfig();
        $result = $this->service->runChecks($config, true);
        $this->json(200, ['ok' => true, 'result' => $result]);
        return true;
    }

    private function dashboardData(): bool
    {
        if (!$this->requireAdminJson()) {
            return true;
        }
        $config = $this->service->loadConfig();
        $this->json(200, $this->service->dashboardPayload($config));
        return true;
    }
    ...
    private function requireAdminPost(): bool
    {
        ...
        if (
            !Security::rateLimit(
                'traffic_alerts_admin:' . Security::clientIpMasked(),
                60,
                30,
            )
        ) {
