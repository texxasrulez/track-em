...
(function (global) {
  ...
  function log() {
    try {
      var a = [LOG_PREFIX, "color:#4caf50;font-weight:700", "color:inherit"];
  ...
  function warn() {
    try {
      var a = [LOG_PREFIX, "color:#ff9800;font-weight:700", "color:inherit"];
      a.push.apply(a, arguments);
  ...
  function err() {
    try {
      var a = [LOG_PREFIX, "color:#e53935;font-weight:700", "color:inherit"];
      a.push.apply(a, arguments);
  ...
  function mount(el, options) {
    ...
    var defaultZoom =
      typeof options.defaultZoom === "number" ? options.defaultZoom : 2;
    var initialCenter = Array.isArray(options.center)
      ? options.center
      : [20, 0];
    var capZoom =
      typeof options.maxInitialZoom === "number" ? options.maxInitialZoom : 3;

    var map = L.map(el, { scrollWheelZoom: true, worldCopyJump: true });
    var tiles =
      options.tiles && typeof options.tiles.url === "string"
        ? options.tiles
        : {
    ...
    if (remember) {
      ...
      if (saved) {
        ...
        restored = true;
        log("restored view", saved);
      }
    }
    if (!restored) {
      map.setView(initialCenter, defaultZoom);
    }
    ...
    if (!global.TEWidgets.visitors) {
      global.TEWidgets.visitors = function (el, opts, data) {
        ...
        if (data && Array.isArray(data.items)) {
          opts.points = toPointsFromGeo(data.items);
        }
  ...
  function autoInit(forceCreate) {
    ...
    if (!el) {
      log("no container yet; observing");
      return;
    }
    ...
    fetchPoints(base, function (_err, pts) {
      ...
      if (target && target.id !== "te-visitors-map") {
        var child = target.querySelector(
          "#te-visitors-map, .map, .leaflet-container, div",
        );
  ...
  global.initVisitorsWidget = function (el, options) {
    log("manual init");
