...
trait ApiPluginsAddon
{
    ...
    public function api_plugins_toggle(): void
    {
        ...
        $enabled =
            in_array($s, ["1", "true", "on", "yes"], true) ||
            $raw === 1 ||
            $raw === true;

        if ($key === "") {
            $this->__respond(400, ["ok" => false, "error" => "missing key"]);
            return;
        }
    ...
    public function api_plugins_config_set(): void
    {
        ...
        if (!is_array($cfg)) {
            $this->__respond(400, ["ok" => false, "error" => "invalid config"]);
            return;
        }
        ...
        if (
            class_exists("\\TrackEm\\Core\\Security") &&
            method_exists("\\TrackEm\\Core\\Security", "verifyCsrf")
        ) {
            if (!\TrackEm\Core\Security::verifyCsrf($csrf)) {
                $this->__respond(400, ["ok" => false, "error" => "bad csrf"]);
                return;
            }
        }
    ...
    private function __stateSave(array $st): void
    {
        file_put_contents(
            $this->__statePath(),
            json_encode($st, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES),
        );
    }
