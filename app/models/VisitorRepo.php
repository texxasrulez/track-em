...
class VisitorRepo
{
    ...
    private function init()
    {
        ...
        try {
            $this->pdo->exec(
                "CREATE INDEX IF NOT EXISTS idx_vis_ip ON te_visitors(ip)",
            );
        } catch (\Exception $e) {
        }
        try {
            $this->pdo->exec(
                "CREATE INDEX IF NOT EXISTS idx_vis_ts ON te_visitors(ts)",
            );
        } catch (\Exception $e) {
        }
    }
    ...
    public static function parseUA($ua)
    {
        ...
        if (stripos($ua, "Edg/") !== false || stripos($ua, "Edge/") !== false) {
            ...
            stripos($ua, "MSIE") !== false ||
            stripos($ua, "Trident/") !== false
        ) {
            $browser = "IE";
        }
    ...
    public function log($ip, $ua, $path, $ref, $geo = [])
    {
        [$os, $browser, $device] = self::parseUA($ua);
        $ts = time();
    ...
    {
        $st = $this->pdo->prepare(
            "SELECT * FROM te_visitors WHERE ip = :ip ORDER BY ts DESC LIMIT :limit",
        );
