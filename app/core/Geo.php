...
final class Geo
{
    ...
    private static function lookupIpApi(string $ip, array $cfg): ?array
    {
        ...
        $allowInsecure = array_key_exists("allow_insecure_http", $cfg)
            ? (bool) $cfg["allow_insecure_http"]
            : self::isLegacyIpApiBase($base);
        $scheme = (string) parse_url($base, PHP_URL_SCHEME);
        if ($scheme === "http" && !$allowInsecure) {
            error_log(
                "[Geo] refusing insecure HTTP geolocation provider; configure HTTPS or set geo.allow_insecure_http=true",
            );
            return null;
        }
    ...
    private static function isLegacyIpApiBase(string $base): bool
    {
        ...
        return $host === "ip-api.com" && ($path === "/json" || $path === "");
    }
    ...
    private static function lookupMaxMindWeb(string $ip, array $cfg): ?array
    {
        ...
        $headers = [
            "Authorization: Basic " .
            base64_encode($accountId . ":" . $license),
    ...
    private static function mmdbToResult(?array $r): ?array
    {
        ...
        if (!empty($r["city"]["names"])) {
            $city =
                (string) ($r["city"]["names"]["en"] ??
                    reset($r["city"]["names"]));
        }
        $country = "";
        if (!empty($r["country"]["names"])) {
            $country =
                (string) ($r["country"]["names"]["en"] ??
                    reset($r["country"]["names"]));
        }
        return [
            "lat" => (float) $lat,
            "lon" => (float) $lon,
     ...
     */
    public static function downloadGeoLite2(
        string $license,
        string $destDir,
    ): array {
        if (!is_dir($destDir)) {
        ...
        if ($data === false || strlen($data) < 1024) {
            return [
                "ok" => false,
                "msg" => "Download failed (bad key or network)",
                "path" => null,
            ];
        }
        ...
        if (!is_file($tmpTar)) {
            ...
            if (is_string($altTar) && is_file($altTar)) {
                @rename($altTar, $tmpTar);
            }
        }
        if (!is_file($tmpTar)) {
            return [
                "ok" => false,
                "msg" => "Could not create TAR from GZ",
                "path" => null,
            ];
        }
        ...
        try {
            ...
            foreach (new \RecursiveIteratorIterator($pharTar) as $file) {
                ...
                if (
                    preg_match('/GeoLite2-City\\.mmdb$/', $file->getFilename())
                ) {
            ...
            $tmpExtract =
                $destDir . "/.geo_extract_" . bin2hex(random_bytes(3));
            ...
            foreach ($it as $f) {
                if (
                    $f->isFile() &&
                    preg_match('/GeoLite2-City\\.mmdb$/', $f->getFilename())
                ) {
                    $mmdbPath = $f->getPathname();
                    break;
                }
            }
            if ($mmdbPath === null) {
                return [
                    "ok" => false,
                    "msg" => "Extracted, but no MMDB found",
                    "path" => null,
                ];
            }
            ...
            if (!is_file($final)) {
                return ["ok" => false, "msg" => "Move failed", "path" => null];
            }
            return ["ok" => true, "msg" => "Downloaded", "path" => $final];
        } catch (\Throwable $e) {
            return [
                "ok" => false,
                "msg" => "Extraction failed: " . $e->getMessage(),
                "path" => null,
            ];
        }
    }
