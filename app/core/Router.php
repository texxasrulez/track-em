    ...
    public function dispatch(): void
    {
        ...
        switch ($p) {
            ...
            case "api.geo.test":
                ...
                break;
            case "api.geo.download":
                (new \TrackEm\Controllers\ApiController())->geoDownload();
                break;

            case "api.plugins.list":
                (new \TrackEm\Controllers\ApiController())->api_plugins_list();
                break;
            case "api.plugins.configs":
                (new \TrackEm\Controllers\ApiController())->pluginConfigs();
                break;
            case "api.plugins.install":
                (new \TrackEm\Controllers\ApiController())->api_plugins_install();
                break;
            case "api.plugins.toggle":
                (new \TrackEm\Controllers\ApiController())->api_plugins_toggle();
                break;
            case "api.plugins.remove":
                (new \TrackEm\Controllers\ApiController())->api_plugins_remove();
                break;
            case "api.plugins.config.set":
                (new \TrackEm\Controllers\ApiController())->api_plugins_config_set();
                break;
            case "api.plugins.asset":
                (new \TrackEm\Controllers\ApiController())->api_plugins_asset();
                break;

            default:
                if (PluginDispatcher::dispatch((string) $p)) {
                    break;
                }
