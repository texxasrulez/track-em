...
final class Security
{
    ...
    {
        try {
            $value = \TrackEm\Core\Config::instance()->get(
    ...
    private static function isTrustedProxy(string $ip): bool
    {
        ...
        foreach (self::trustedProxyIps() as $candidate) {
    ...
    public static function clientIp(int $maskBits = 0): string
    {
        ...
        if (
            ...
            $parts = explode(".", $ip);
            $maskOctets = intdiv($maskBits, 8);
            for ($i = 4 - $maskOctets; $i < 4; $i++) {
                $parts[$i] = "0";
            }
    ...
    public static function clientIpMasked(): string
    {
        ...
        if (strpos($ip, ":") !== false) {
            try {
                $bits = (int) \TrackEm\Core\Config::instance()->get(
                    "privacy.ipv6_mask_bits",
                    64,
                );
            } catch (\Throwable $e) {
                $bits = 64;
            ...
            $b = 0;
            try {
                $b = (int) \TrackEm\Core\Config::instance()->get(
                    "privacy.ip_mask_bits",
                    16,
                );
            } catch (\Throwable $e) {
                $b = 16;
    ...
    public static function rateLimit(
        ...
        if (is_file($path)) {
            ...
            if ($raw !== false) {
                ...
                if (is_array($dec) && isset($dec["w"], $dec["hits"])) {
                    $data = $dec;
                }
            }
        }
        if ($now - (int) $data["w"] >= $windowSec) {
            $data["w"] = $now;
            $data["hits"] = 0;
        }
        if ((int) $data["hits"] >= $maxEvents) {
            return false;
        }
    ...
    public static function emitSecurityHeaders(): void
    {
        try {
            ...
            if (!$cfg->get("security_headers.enabled", true)) {
                return;
            }
