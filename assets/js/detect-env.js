...
export async function detectBrowser() {
  ...
  if (uaData && Array.isArray(uaData.brands)) {
    for (const b of uaData.brands) {
      ...
      if (n.includes("opera") || n.includes("opr")) {
        brandName = "Opera";
        break;
      }
  ...
  const isChromeFamily =
    /chrome\//.test(ua) || /crios\//.test(ua) || /chromium\//.test(ua);
  let browser = "Unknown";
  if (braveViaNavigator || brandName === "Brave" || /\bbrave\//.test(ua))
    browser = "Brave";
  ...
  if (uaData && typeof uaData.getHighEntropyValues === "function") {
    try {
      ...
      if (Array.isArray(fullVersionList)) {
        ...
        const match =
          fullVersionList.find((b) =>
            (b.brand || "")
              .toLowerCase()
              .includes((wanted || "").toLowerCase()),
          ) ||
          fullVersionList.find(
            (b) => !/^not/.test((b.brand || "").toLowerCase()),
          );
...
export async function detectEnv() {
  const [browser, os] = await Promise.all([detectBrowser(), detectOS()]);
  return { browser, os };
}
(async function attach() {
  try {
    ...
    if (env.browser?.name)
      root.dataset.browser = env.browser.name.toLowerCase();
    if (env.os?.name) root.dataset.os = env.os.name.toLowerCase();
    if (env.os?.distro)
      root.dataset.distro = env.os.distro
        .toLowerCase()
        .replace(/\s+/g, "-")
        .replace(/[!_]/g, "-");
  } catch {}
