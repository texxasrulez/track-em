(function () {
  try {
    ...
    function setStoredConsent(value) {
      try {
        localStorage.setItem("te_consent", value);
      } catch (_) {}
    }
    ...
    fetchJSON(apiUrl)
      .then(function (data) {
        ...
        if (!enabled) return;
        var requireConsent =
          typeof cfg.require_consent === "undefined"
            ? true
            : !!cfg.require_consent;
      ...
      .catch(function () {
        if (window.TE_CONSENT_FORCE) renderBanner({});
      });
    ...
    function renderBanner(cfg) {
      ...
      var text =
        cfg.message ||
        "We use cookies and similar tech to analyze traffic. Choose allow or deny.";
      ...
      function mkBtn(label, bg, color) {
        ...
        b.style.boxShadow = "0 1px 2px rgba(0,0,0,.2)";
      ...
      function decide(val) {
        ...
        setStoredConsent(val);
        new Image().src =
          BASE +
          "/track.php?event=consent&value=" +
