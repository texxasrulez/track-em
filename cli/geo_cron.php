...
function logln(string $s)
{
    echo "[" . date("c") . "] " . $s . PHP_EOL;
}
...
try {
    ...
    try {
        $pdo->query("SELECT lat, lon, city, country FROM visits LIMIT 0");
    } catch (\Throwable $e) {
    ...
    $since = time() - 7 * 86400;
    $st = $pdo->prepare(
        "SELECT id, ip FROM visits WHERE ts >= ? AND (lat IS NULL OR lon IS NULL) ORDER BY id DESC LIMIT 2000",
    );
    $st->execute([$since]);
    $rows = $st->fetchAll();
    ...
    foreach ($rows as $r) {
        ...
        if ($geoRow) {
            $up = $pdo->prepare(
                "UPDATE visits SET lat=?, lon=?, city=?, country=? WHERE id=?",
            );
            $up->execute([
                $geoRow["lat"],
                $geoRow["lon"],
