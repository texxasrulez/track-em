...
function usage(int $exit = 0): void
{
    ...
    Notes:
      ...
      • Later runs translate only missing keys added to the source locale.
      • Existing translations are left untouched unless --force is used.
...
if ($apiKey === "" && !$dryRun) {
    fwrite(
        STDERR,
        "ERROR: DEEPL_API_KEY is not set. Export it or run with --dry-run for previews.\n",
    );
    exit(2);
}
...
if (!$apiBase || trim($apiBase) === "") {
    $apiBase = str_contains($apiKey, ":fx")
...
$bootstrap = empty($state["source_hash"] ?? []);

// Discover targets
$files = array_values(
    array_filter(
        glob($i18nDir . "/*.php"),
        fn($f) => basename($f) !== "{$source}.php",
    ),
);
if (is_array($only) && !empty($only)) {
    $files = array_values(
        array_filter($files, function ($f) use ($only) {
        ...
        if (is_array($data)) {
            foreach ($data as $item) {
                if (isset($item["language"])) {
                    $supportedTargets[] = strtoupper($item["language"]);
                }
            }
        } else {
            $deepLError =
                "languages_non_json_http_{$http}: " . substr($resp, 0, 200);
        }
    }
    curl_close($curl);
}
...
foreach ($files as $file) {
    ...
    if (
        !$targetCode ||
        !in_array($targetCode, $supportedTargets, true) ||
        str_starts_with($targetCode, "EN")
    ) {
        $report["skipped_locales"][] = [
            "locale" => $locale,
            "reason" => "unsupported or English",
            "mapped" => $targetCode,
        ];
        continue;
    }
    ...
    if (empty($toUpdate)) {
        $report["updated_locales"][] = [
            "locale" => $locale,
            "updates" => 0,
            "note" => $bootstrap
                ? "no missing keys on bootstrap"
                : "no new keys to translate",
        ];
        continue;
    }
...
fwrite(
    STDOUT,
    "[OK] Locales processed. Total keys translated: {$totalTranslated}\n",
);
...
if ($dryRun) {
    ...
    fwrite(
        STDOUT,
        "DeepL live translation used for missing keys via {$apiBase}.\n",
    );
}
